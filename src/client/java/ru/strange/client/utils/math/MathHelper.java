...
public class MathHelper {
    ...
    private static double randomDoubleInRange(double min, double max) {
        ...
        return lower == upper ? lower : ThreadLocalRandom.current().nextDouble(lower, upper);
    }

    private static float randomFloatInRange(float min, float max) {
        float lower = Math.min(min, max);
        float upper = Math.max(min, max);
        return lower == upper ? lower : (float) ThreadLocalRandom.current().nextDouble(lower, upper);
    }

    private static int randomIntInclusive(int min, int max) {
        int lower = Math.min(min, max);
        int upper = Math.max(min, max);
        return lower == upper ? lower : ThreadLocalRandom.current().nextInt(lower, upper + 1);
    }

    private static int randomIntExclusive(int min, int max) {
        int lower = Math.min(min, max);
        int upper = Math.max(min, max);
        return lower == upper ? lower : ThreadLocalRandom.current().nextInt(lower, upper);
    }
    ...
    public static int getRandomInRange(int max, int min) {
        return randomIntExclusive(min, max);
    }
    ...
    public static float randomFloat(float f2, float f3) {
        return randomFloatInRange(f2, f3);
    }
    ...
    public static float lerpRandom(float a, float b, float f, float random) {
        float randomFactor = randomFloatInRange(0.0f, random);
