...
public class RenderUtil implements Helper {
    ...
    private static final int CLIENT_RECT_NEON = 0xCC1A0033;
    ...
    public static void drawClientRect(DrawContext ctx, float x, float y, float width, float height) {
        ...
        } else if (Optimization.shouldSkipHudBlur()) {
            return;
        } else if (theme != Theme.BLACK && theme != Theme.WHITE) {
    ...
    public static class LiquidGlass {
        ...
        private static void drawGlass(DrawContext ctx, float x, float y, float width, float height,
            ...
            // Ensure blur is always visible even at low alpha by using minimum 0.3 alpha for blur texture
            float blurAlpha = Math.max(0.3f, clampedAlpha);
            LegacyBlurUtil.render(ctx, x, y,
                    new SizeState(width, height), radius,
                    new QuadColorState(ColorUtil.replAlpha(0xFFFFFFFF, (int) (255.0f * blurAlpha))),
                    1, blurRadius);
