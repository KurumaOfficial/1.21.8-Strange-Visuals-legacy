...
public class NewGuiState {
    ...
    public static final ScrollUtil themeScroll = new ScrollUtil();

    public static float openAnimation = 0f;
    public static boolean closing = false;

    public static float getPanelContentStartY(int categoryIndex) {
        return panelY[categoryIndex] + HEADER_HEIGHT;
    }

    public static float getPanelContentEndY(int categoryIndex) {
        return panelY[categoryIndex] + PANEL_HEIGHT - 6f;
    }
    public static float[] panelScroll = new float[CATEGORIES.length];

    public static boolean isModuleInsidePanel(int categoryIndex, float moduleY) {
        float start = getPanelContentStartY(categoryIndex);
        float end = getPanelContentEndY(categoryIndex);
        return moduleY >= start && moduleY <= end;
    }

    public static float clampModuleY(int categoryIndex, float y) {
        float start = getPanelContentStartY(categoryIndex);
        float end = getPanelContentEndY(categoryIndex);
    ...
    public static float getRightSearchWidth() {
        return SEARCH_WIDTH;
    }

    public static float getRightSearchHeight() {
        return SEARCH_HEIGHT;
    }

    public static float getRightSearchX() {
    ...
    public static float getRightSearchY() {
        return panelY[0] + PANEL_HEIGHT + 12f;
    }

    public static float getRightSearchCenterY() {
        return getRightSearchY() + 40f;
    }
    ...
    public static float promoMessageAlpha = 0f;
    public static boolean promoInputFocused = false;
    // -- end promo state --
    ...
    public static boolean themeDropdownOpened = false;
    ...
    public static List<Module> getVisibleModules(Category category) {
        ...
        List<Module> cached = visibleModulesCache.get(category);
        // Fallback: если кэш пустой, попробуем получить все модули без фильтрации
    ...
    public static float getThemeBarY() {
        return currentScreenHeight - 30f;
    }
    ...
    public static float themeDropdownAnimation = 0f;
    ...
    public static float getPanelSlideOffset(int index) {
        float sizing = panelSizing[index];
        if (sizing < 0.001f) {
            return 0f;
        }
        return closing ? (1f - sizing) * -40f : (1f - sizing) * 60f;
    }

    public static float getPanelRenderScale(int index) {
        float sizing = panelSizing[index];
        if (sizing < 0.001f) {
            return 1f;
        }
        return closing ? 0.88f + sizing * 0.12f : 0.92f + sizing * 0.08f;
    }

    /** Maps screen mouse coordinates into the panel's layout space (inverse of render transform). */
    public static double[] toPanelMouse(int index, double mouseX, double mouseY) {
        float sizing = panelSizing[index];
        if (sizing < 0.001f) {
            return new double[]{Double.NaN, Double.NaN};
        }

        float px = getRenderedPanelX(index);
        float py = panelY[index] + getPanelSlideOffset(index);
        float scale = getPanelRenderScale(index);
        return GuiMouseUtil.panelLocalMouse(mouseX, mouseY, px, py, PANEL_WIDTH, PANEL_HEIGHT, scale);
    }
    ...
    public static void resetState() {
        ...
        themeScroll.reset(); // Добавь эту строчку
        themeDropdownAnimation = 0f;
