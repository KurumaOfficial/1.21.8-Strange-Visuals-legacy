...
public class GuiRenderSettings extends GuiScreen {
    ...
    public static void renderSettings(DrawContext ctx, java.util.List<Setting> settings, float x, float y, double mouseX, double mouseY) {
        ...
        for (Setting setting : settings) {
            ...
            if (setting instanceof SliderSetting) {
                ...
                String sliderLabel = truncateToWidth(setting.getDisplayName(), 5, 68f);
                String sliderValue = String.valueOf(s.get());
                FontDraw.drawText(FontDraw.FontType.MEDIUM, ctx, sliderLabel, xSettings + 5, ySettings + 8, 5, RenderUtil.ColorUtil.getTextColor(1,1));
                float valueWidth = FontDraw.getWidth(FontDraw.FontType.MEDIUM, sliderValue, 5);
                FontDraw.drawText(FontDraw.FontType.MEDIUM, ctx, sliderValue, xSettings + 98f - valueWidth, ySettings + 8, 5, RenderUtil.ColorUtil.getTextColor(1,1));
    ...
    private static String truncateToWidth(String text, int fontSize, float maxWidth) {
        if (text == null || text.isEmpty()) {
            return "";
        }
        if (FontDraw.getWidth(FontDraw.FontType.MEDIUM, text, fontSize) <= maxWidth) {
            return text;
        }
