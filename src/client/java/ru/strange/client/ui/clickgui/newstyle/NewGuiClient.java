...
public class NewGuiClient extends Screen implements Helper {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (handlePromoKey(keyCode)) return true;
    ...
    public boolean charTyped(char codePoint, int modifiers) {
        if (NewGuiState.promoInputFocused) {
            if (!Character.isISOControl(codePoint)) {
                NewGuiState.promoInputText += codePoint;
            }
            return true;
        }
    ...
    private boolean handlePromoKey(int keyCode) {
        if (!NewGuiState.promoInputFocused) return false;

        if (keyCode == GLFW.GLFW_KEY_BACKSPACE) {
            if (!NewGuiState.promoInputText.isEmpty()) {
                NewGuiState.promoInputText = NewGuiState.promoInputText.substring(0, NewGuiState.promoInputText.length() - 1);
            }
            return true;
        }
        if (keyCode == GLFW.GLFW_KEY_ENTER) {
            NewGuiMouseHandler.applyPromo();
