...
public class ItemShaderProfilesScreen extends Screen {
    ...
    private static final ShaderThemePreset[] SELECTABLE_PRESETS = ShaderThemePreset.selectablePresets();
    ...
    private final Map<String, ItemEntry> itemEntriesById = new HashMap<>();
    ...
    protected void init() {
        ...
        searchField.setChangedListener(value -> {
            ...
            refreshSelectedThemeVisibility();
        });
    ...
    private void buildItemList() {
        ...
        for (Item item : Registries.ITEM) {
            ...
            ItemEntry entry = new ItemEntry(itemId, stack, displayName, searchKey);
            allItems.add(entry);
            itemEntriesById.put(itemId, entry);
        }
    ...
    private void renderThemeGrid(DrawContext context, int mouseX, int mouseY, RectArea area,
        ...
        int totalRows = (int) Math.ceil(SELECTABLE_PRESETS.length / (double) THEME_COLUMNS);
        ...
        for (int index = 0; index < SELECTABLE_PRESETS.length; index++) {
            ShaderThemePreset preset = SELECTABLE_PRESETS[index];
    ...
    private ItemEntry getSelectedEntry() {
        return selectedItemId == null ? null : itemEntriesById.get(selectedItemId);
    }
    ...
    private void ensureSelectedThemeVisible(Map<String, ItemShaderProfiles.ShaderProfile> profiles) {
        ...
        int selectedIndex = -1;
        for (int index = 0; index < SELECTABLE_PRESETS.length; index++) {
            if (SELECTABLE_PRESETS[index] == selectedTheme) {
                selectedIndex = index;
                break;
            }
        }
    ...
    private void refreshSelectedThemeVisibility() {
    ...
    private ShaderThemePreset resolveThemeAt(double mouseX, double mouseY, RectArea grid) {
        ...
        for (int index = 0; index < SELECTABLE_PRESETS.length; index++) {
            ...
            if (GuiScreen.isHovered(mouseX, mouseY, cellX, cellY, cellWidth, THEME_H)) {
                return SELECTABLE_PRESETS[index];
            }
        }
        return null;
    }
