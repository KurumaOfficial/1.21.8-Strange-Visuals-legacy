...
public class ItemShaderProfilesScreen extends Screen {
    ...
    private boolean isInsidePanel(double mouseX, double mouseY) {
        return GuiScreen.isHovered(mouseX, mouseY, panelX, panelY, PANEL_W, PANEL_H);
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String profileChip = localized("Профилей: ", "Profiles: ") + ItemShaderProfiles.snapshot().size();
        ...
        RectArea left = new RectArea(panelX + PAD, panelY + HEADER_H + 8, LEFT_W, PANEL_H - HEADER_H - FOOTER_H - 16);
        RectArea right = new RectArea(left.right() + GAP, left.y(), PANEL_W - LEFT_W - PAD * 2 - GAP, left.height());
        RectArea preview = new RectArea(right.x(), right.y(), right.width(), 96);
        RectArea themes = new RectArea(right.x(), preview.bottom() + GAP, right.width(), right.height() - preview.height() - GAP);
        RectArea footer = new RectArea(panelX + PAD, panelY + PANEL_H - FOOTER_H - 4, PANEL_W - PAD * 2, FOOTER_H);
        ...
        renderItemList(context, mouseX, mouseY, listArea(left), text, muted, accent, line);
        renderPreview(context, preview, text, muted, accent, line);
        renderThemeGrid(context, mouseX, mouseY, themes, text, accent);
        renderFooter(context, mouseX, mouseY, footer, text, muted, accent, line);
    ...
    private void renderItemList(DrawContext context, int mouseX, int mouseY, RectArea area, int text, int muted, int accent, int line) {
        int contentHeight = filteredItems.size() * ROW_H;
        ...
        for (int index = 0; index < filteredItems.size(); index++) {
            ...
            boolean overridden = ItemShaderProfiles.find(entry.itemId()) != null;
    ...
    private void renderPreview(DrawContext context, RectArea area, int text, int muted, int accent, int line) {
        ...
        ItemShaderProfiles.ShaderProfile profile = ItemShaderProfiles.find(selectedEntry.itemId());
        ShaderThemePreset preset = profile == null ? ShaderThemePreset.COSMOS : profile.resolveTheme(ShaderThemePreset.COSMOS);
        ...
        String themeLabel = profile == null
                ? localized("Глобальная тема", "Global theme")
                : ModLocalization.raw(preset.displayName());
        ...
        drawChip(context, area.x() + 66 + chipWidth(themeLabel) + 8, area.y() + 48,
                profile == null ? localized("Без override", "No override") : localized("Override активен", "Override active"),
                alpha(sectionAltColor(), 220), line, text);
    }

    private void renderThemeGrid(DrawContext context, int mouseX, int mouseY, RectArea area, int text, int accent) {
        ...
        ShaderThemePreset selectedTheme = getSelectedTheme();
    ...
    private void renderFooter(DrawContext context, int mouseX, int mouseY, RectArea footer, int text, int muted, int accent, int line) {
        ...
        drawActionButton(context, clear, mouseX, mouseY, localized("Сбросить профиль", "Clear Profile"), accent, ItemShaderProfiles.find(selectedItemId) != null);
    ...
    private ShaderThemePreset getSelectedTheme() {
        if (selectedItemId == null) {
            return ShaderThemePreset.COSMOS;
        }

        ItemShaderProfiles.ShaderProfile profile = ItemShaderProfiles.find(selectedItemId);
        return profile == null ? ShaderThemePreset.COSMOS : profile.resolveTheme(ShaderThemePreset.COSMOS);
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        RectArea preview = new RectArea(right.x(), right.y(), right.width(), 96);
        RectArea themes = new RectArea(right.x(), preview.bottom() + GAP, right.width(), right.height() - preview.height() - GAP);
        RectArea themeGrid = new RectArea(themes.x() + 10, themes.y() + 36, themes.width() - 20, themes.height() - 46);
        ...
        if (GuiScreen.isHovered(mouseX, mouseY, clear.x(), clear.y(), clear.width(), clear.height())) {
            if (ItemShaderProfiles.find(selectedItemId) != null) {
