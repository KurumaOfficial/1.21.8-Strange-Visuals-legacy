...
public class FontRenderer {
    private static final ThreadLocal<GlyphBuffer> MEASURE_BUFFER = ThreadLocal.withInitial(GlyphBuffer::new);
    ...
    public float getWidth(@NotNull String text, int size) {
        Font font = family.getSize(size);
        GlyphBuffer buffer = measurementBuffer();
    ...
    public float getAscent(int size) {
        return family.getSize(size).ascent();
    }
    ...
    private static GlyphBuffer measurementBuffer() {
        GlyphBuffer buffer = MEASURE_BUFFER.get();
        buffer.clear();
        return buffer;
    }
