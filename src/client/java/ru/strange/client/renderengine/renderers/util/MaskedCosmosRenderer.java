...
public final class MaskedCosmosRenderer {
    ...
    public void render(MaskDrawer drawer,
        ...
        try (BufferAllocator allocator = new BufferAllocator(1 << 18)) {
            VertexConsumerProvider.Immediate immediate = VertexConsumerProvider.immediate(allocator);
            drawer.draw(immediate);
            immediate.draw();
        }

        captureSceneAfter(framebuffer, width, height);
        restoreScene(framebuffer, width, height);
    ...
    private void captureSceneAfter(Framebuffer framebuffer, int width, int height) {
        CommandEncoder encoder = RenderSystem.getDevice().createCommandEncoder();
        encoder.copyTextureToTexture(framebuffer.getColorAttachment(), sceneAfterTex, 0, 0, 0, 0, 0, width, height);
    }

    private void restoreScene(Framebuffer framebuffer, int width, int height) {
        CommandEncoder encoder = RenderSystem.getDevice().createCommandEncoder();
