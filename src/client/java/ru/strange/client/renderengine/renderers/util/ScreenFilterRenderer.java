...
public final class ScreenFilterRenderer {
    ...
    private boolean hasVisibleEffect() {
        return Math.abs(brightness) > EPSILON
                || Math.abs(contrast - 1.0f) > EPSILON
                || Math.abs(saturation - 1.0f) > EPSILON
                || Math.abs(exposure - 1.0f) > EPSILON
                || Math.abs(gamma - 1.0f) > EPSILON
                || Math.abs(vignette) > EPSILON
                || Math.abs(grain) > EPSILON
                || Math.abs(tintIntensity) > EPSILON;
    }
