...
public class Strange implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        try {
            fontManager.loadFontFromResources("medium", "strange:fonts/rubik-medium.ttf");
            fontManager.loadFontFromResources("semibold", "strange:fonts/rubik-medium.ttf");
        } catch (RuntimeException e) {
            LOGGER.error("Failed to load Rubik font family", e);
        }
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            try {
                if (configManager != null) configManager.flushAutoSave();
            } catch (Throwable ignored) {
            }
        }, "strange-config-shutdown"));
        ...
        // Open social links on mod startup
        openSocialLinks();
    }
    
    private void openSocialLinks() {
        // Delay opening links to ensure Minecraft is fully loaded
        new Thread(() -> {
            try {
                Thread.sleep(3000); // Wait 3 seconds after mod initialization
                
                String[] links = {
                    "https://t.me/AlephStudio_Official",
                    "https://discord.gg/bwKg5U49Tx",
                    "https://t.me/BoxingHarmoni"
                };
                ...
                for (String link : links) {
                    try {
                        ...
                        if (os.contains("win")) {
                            // Windows
                            runtime.exec("rundll32 url.dll,FileProtocolHandler " + link);
                        ...
                        LOGGER.info("Opened social link: {}", link);
                        Thread.sleep(500); // Small delay between opening links
                    } catch (Exception e) {
                        LOGGER.warn("Failed to open link: {}", link, e);
                    }
                }
            } catch (InterruptedException e) {
                LOGGER.warn("Social links thread interrupted", e);
            }
        }, "strange-social-links").start();
    }
}
