...
public final class AltStartupSessionSync {
    ...
    private static final AltManagerScreen.AltAccountStore ACCOUNT_STORE = new AltManagerScreen.AltAccountStore(ALTS_FILE);
    private static final AltManagerScreen.AltSessionService SESSION_SERVICE = new AltManagerScreen.AltSessionService();

    private static AltManagerScreen.AltAccountStore.LoadedAccounts cachedAccounts;
    ...
    public static void applyIfPossible(MinecraftClient client) {
        ...
        AltManagerScreen.AltAccountStore.LoadedAccounts loaded = loadAccounts();
        ...
        AltManagerScreen.AltSessionService.SwitchResult result = SESSION_SERVICE.switchToOfflineProfile(client, targetName);
        if (result == AltManagerScreen.AltSessionService.SwitchResult.INVALID_NAME) {
            refresh();
        }
    }
    ...
    private static AltManagerScreen.AltAccountStore.LoadedAccounts loadAccounts() {
    ...
    private static String resolvePreferredName(AltManagerScreen.AltAccountStore.LoadedAccounts loaded, String currentName) {
        ...
        if (currentName != null) {
            for (AltManagerScreen.AltAccount account : loaded.active()) {
