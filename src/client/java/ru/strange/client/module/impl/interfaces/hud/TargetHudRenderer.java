...
public final class TargetHudRenderer {
    ...
    private static final float CIRCLE_W = 108.0F;
    private static final float H = 34.0F;
    private static final float HEAD_SIZE = 18.0F;
    private static final float BAR_H = 4.0F;
    private static final float ARMOR_SCALE = 0.62F;
    private static final float ARMOR_STEP = 9.8F;
    private static final float HEALTH_CIRCLE_SIZE = 24.0F;
    private static final int HEALTH_CIRCLE_SEGMENTS = 28;
    ...
    private static final Identifier PREVIEW_TARGET_FACE_TEXTURE = Identifier.of("minecraft", "textures/entity/skeleton/skeleton.png");
    ...
    public void render(DrawContext ctx, float x, float y) {
        ...
        LivingEntity renderTarget = preview ? null : (this.target != null ? this.target : this.displayTarget);
        if (!preview && renderTarget == null) {
            return;
        }

        boolean circleHealth = this.usesCircleHealth();
        float width = this.getWidth();
        float height = this.getHeight();
        float headX = x + 5.0F;
        float headY = y + (height - HEAD_SIZE) * 0.5F;

        RenderUtil.drawClientRect(ctx, x, y, width, height);
        Round.draw(
                ctx,
                x,
                y,
        ...
        if (preview) {
            this.drawFaceTexture(ctx,
                    PREVIEW_TARGET_FACE_TEXTURE,
                    headX + 2.0F, headY + 2.0F,
                    (int) (HEAD_SIZE - 4.0F),
                    8.0F, 8.0F,
                    64, 32,
                    false,
        ...
        float nameY = y + 10.6F;
        float armorY = y + 18.1F;

        float contentRight = x + width - 6.0F;
        float maxNameWidth;
        float hpX = contentRight;
        String hpText = circleHealth ? this.compactHealthText(health) : oneDecimal(health);

        if (circleHealth) {
            float circleX = contentRight - HEALTH_CIRCLE_SIZE;
            float circleY = y + (height - HEALTH_CIRCLE_SIZE) * 0.5F;
            maxNameWidth = Math.max(18.0F, circleX - textX - 4.0F);
            this.renderHealthCircle(ctx, circleX, circleY, HEALTH_CIRCLE_SIZE, progress, alpha);
            FontDraw.drawCenter(
                    FontType.MEDIUM,
                    ctx,
                    hpText,
                    circleX + HEALTH_CIRCLE_SIZE * 0.5F,
                    this.centeredTextY(circleY, HEALTH_CIRCLE_SIZE, FontType.MEDIUM, 4) - 0.5F,
                    4,
                    this.textTheme(alpha),
                    false
            );
        } else {
            hpX = contentRight - FontDraw.getWidth(FontType.MEDIUM, hpText, 5);
            maxNameWidth = Math.max(18.0F, hpX - textX - 4.0F);
            FontDraw.drawText(FontType.MEDIUM, ctx, hpText, hpX, nameY - 0.2F, 5, this.textTheme(alpha));
        }
        ...
        FontDraw.drawText(FontType.MEDIUM, ctx, name, textX, nameY, 5, this.textTheme(alpha));
        this.renderArmorRow(ctx, preview, renderTarget, textX, armorY);

        if (!circleHealth) {
            float barX = x + 5.0F;
            float barY = y + height - 6.0F;
    ...
    public float getWidth() {
        return this.usesCircleHealth() ? CIRCLE_W : LINE_W;
    }
    ...
    private void renderArmorRow(DrawContext ctx, boolean preview, LivingEntity renderTarget, float armorX, float armorY) {
        for (int i = 0; i < 4; ++i) {
            ItemStack stack;
            if (preview) {
                stack = switch (i) {
    ...
    private void renderHealthCircle(DrawContext ctx, float x, float y, float size, float progress, float alpha) {
        float centerX = x + size * 0.5F;
        float centerY = y + size * 0.5F;
        float dotSize = 2.6F;
        float radius = size * 0.5F - dotSize * 0.9F;
        ...
        for (int i = 0; i < HEALTH_CIRCLE_SEGMENTS; i++) {
            float segment = i / (float) HEALTH_CIRCLE_SEGMENTS;
            double angle = -Math.PI / 2.0 + segment * Math.PI * 2.0;
            float dotX = centerX + (float) Math.cos(angle) * radius - dotSize * 0.5F;
            float dotY = centerY + (float) Math.sin(angle) * radius - dotSize * 0.5F;
            boolean active = segment <= progress + (1.0F / HEALTH_CIRCLE_SEGMENTS);
            int color = active
                    ? this.barFillTheme(alpha)
                    : ColorUtil.replAlpha(ColorUtil.getTextColor(1, 1), (int) (45.0F * alpha));
            Round.draw(ctx, dotX, dotY, dotSize, dotSize, dotSize * 0.5F, color);
        }
    }
    ...
    private float centeredTextY(float y, float height, FontType fontType, int size) {
        float fontHeight = FontDraw.getHeight(fontType, size);
        float ascent = FontDraw.getAscent(fontType, size);
        return y + (height - fontHeight) * 0.5F + ascent;
    }
