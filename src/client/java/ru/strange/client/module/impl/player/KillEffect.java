...
@IModule(
        name = "Kill Effect",
        description = "kill and totem pop visuals with lightning, particles and shockwaves",
...
public class KillEffect extends Module {
    ...
    public void onRender(EventRender3D event) {
        if (mc.world == null || mc.player == null || (particles.isEmpty() && bursts.isEmpty() && shockwaves.isEmpty())) {
            return;
        }
        ...
        try {
            if (!particles.isEmpty()) {
                ParticleUtil.renderParticles(event.getMatrixStack(), immediate, mc.gameRenderer.getCamera().getPos(), particles, 300, 850, deltaTime);
                particles.removeIf(particle -> particle.time().finished(1200));
            }
    ...
    private void spawnLightning(Vec3d pos, int color, int accentColor) {
        boolean vanillaStyle = lightningStyle.is("Ванильная");
        bursts.add(new LightningBurst(pos, color, accentColor, vanillaStyle));
        if (vanillaStyle) {
