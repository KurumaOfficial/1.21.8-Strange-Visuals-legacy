...
public class Hat extends Module {
    ...
    private void renderFill(VertexConsumerProvider.Immediate immediate, Matrix4f matrix) {
        ...
        for (int i = 0; i < segmentCount; i++) {
            ...
            fillBuffer.vertex(matrix, 0.0f, hatHeight, 0.0f).color(getCenterColor(alphaValue));
            fillBuffer.vertex(matrix, (float) Math.cos(angle1) * hatRadius, 0.0f, (float) Math.sin(angle1) * hatRadius).color(getSegmentColor(i, segmentCount, alphaValue));
            fillBuffer.vertex(matrix, (float) Math.cos(angle2) * hatRadius, 0.0f, (float) Math.sin(angle2) * hatRadius).color(getSegmentColor(i + 1, segmentCount, alphaValue));
        }
    }
    ...
    private void renderMask(VertexConsumer buffer, Matrix4f matrix, int alphaValue) {
        ...
        for (int i = 0; i < segmentCount; i++) {
            ...
            buffer.vertex(matrix, (float) Math.cos(angle1) * hatRadius, 0.0f, (float) Math.sin(angle1) * hatRadius).color(color);
            buffer.vertex(matrix, (float) Math.cos(angle2) * hatRadius, 0.0f, (float) Math.sin(angle2) * hatRadius).color(color);
        }
    }
    ...
    private void renderOutline(VertexConsumerProvider.Immediate immediate, Matrix4f matrix, boolean shaderOutline) {
        ...
        for (int i = 0; i < segmentCount; i++) {
            ...
            lineBuffer.vertex(matrix, (float) Math.cos(angle1) * hatRadius, 0.0f, (float) Math.sin(angle1) * hatRadius).color(lineColor1);
            lineBuffer.vertex(matrix, (float) Math.cos(angle2) * hatRadius, 0.0f, (float) Math.sin(angle2) * hatRadius).color(lineColor2);
        }
    }
