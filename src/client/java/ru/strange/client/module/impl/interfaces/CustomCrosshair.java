...
public class CustomCrosshair extends Module {
    ...
    private void renderCross(DrawContext ctx, float cx, float cy,
        ...
        float[][] lines = {
                {cx - h, cy - gp - length, th, length},
                {cx - h, cy + gp, th, length},
                {cx - gp - length, cy - h, length, th},
                {cx + gp, cy - h, length, th},
        };

        if (outlineOn.get()) {
            for (float[] l : lines) {
                RenderUtil.Rect.draw(ctx,
                        l[0] - 1.0f, l[1] - 1.0f,
                        l[2] + 2.0f, l[3] + 2.0f,
                        outColor);
            }
        ...
        for (float[] l : lines) {
            RenderUtil.Rect.draw(ctx, l[0], l[1], l[2], l[3], color);
        }
    }
    ...
    private void renderBrackets(DrawContext ctx, float cx, float cy,
        ...
        float[][] rects = {
                {cx - off, cy - off, arm, th},
                {cx - off, cy - off, th, arm},
                {cx + off - arm, cy - off, arm, th},
                {cx + off - th, cy - off, th, arm},
                {cx - off, cy + off - th, arm, th},
                {cx - off, cy + off - arm, th, arm},
                {cx + off - arm, cy + off - th, arm, th},
                {cx + off - th, cy + off - arm, th, arm},
        };
        ...
        for (float[] r : rects) {
            RenderUtil.Rect.draw(ctx, r[0], r[1], r[2], r[3], color);
        }
    }
    ...
    private void renderHitMarker(DrawContext ctx, float cx, float cy,
        ...
        for (int i = 0; i < 4; i++) {
            double angle = Math.PI / 4.0 + Math.PI / 2.0 * i;
