...
public class JumpCircle extends Module {
    ...
    private void renderScans(MatrixStack matrices, VertexConsumerProvider.Immediate immediate) {
        ...
        for (SurfaceScan scan : scans) {
            ...
            int segmentCount = Math.max(18, (int) scanSegments.get());
    ...
    private void drawTexturedQuad(VertexConsumer buffer, Matrix4f matrix, Matrix3f normalMatrix, float x, float y, float width, float height, int color) {
        ...
        buffer.vertex(matrix, x1, y1, 0.0f).color(r, g, b, a).texture(0.0f, 1.0f).overlay(OverlayTexture.DEFAULT_UV).light(0xF000F0).normal(normal.x, normal.y, normal.z);
        buffer.vertex(matrix, x2, y1, 0.0f).color(r, g, b, a).texture(1.0f, 1.0f).overlay(OverlayTexture.DEFAULT_UV).light(0xF000F0).normal(normal.x, normal.y, normal.z);
        buffer.vertex(matrix, x2, y2, 0.0f).color(r, g, b, a).texture(1.0f, 0.0f).overlay(OverlayTexture.DEFAULT_UV).light(0xF000F0).normal(normal.x, normal.y, normal.z);
        buffer.vertex(matrix, x1, y2, 0.0f).color(r, g, b, a).texture(0.0f, 0.0f).overlay(OverlayTexture.DEFAULT_UV).light(0xF000F0).normal(normal.x, normal.y, normal.z);
    }
