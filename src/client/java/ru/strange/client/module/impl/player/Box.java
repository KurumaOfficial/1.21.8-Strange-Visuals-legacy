...
public class Box extends Module {
    ...
    public static ModeSetting shaderTheme = new ModeSetting("Shader Theme", ShaderThemePreset.COSMOS.displayName(), ShaderThemePreset.names())
        ...
        public static BooleanSetting shaderFill = new BooleanSetting("Shader Fill", false)
            .hidden(() -> !usesThemeStyle());
    ...
    public Box() {
        addSettings(renderMode, targets, colorStyle, shaderTheme, shaderFill, friendShader, friendShaderTheme, colorSetting, hitColorize, hitColor, range, glowSize);
    }
    ...
    private void renderBox(MatrixStack matrices, VertexConsumerProvider.Immediate immediate, Entity target,
        ...
        if (shaderFill.get() && usesThemeStyle()) {
            String theme = resolveShaderTheme(target);
            double phase = System.currentTimeMillis() * 0.0014 + x * 0.27 + y * 0.18 + z * 0.23;
            gradientColors[0] = RenderUtil.ColorUtil.interpolate(ShaderThemeVisuals.animatedPrimary(theme, phase), baseColor, 0.28);
            gradientColors[1] = RenderUtil.ColorUtil.interpolate(ShaderThemeVisuals.animatedSecondary(theme, phase + 0.65), baseColor, 0.28);
            gradientColors[2] = RenderUtil.ColorUtil.interpolate(ShaderThemeVisuals.animatedPrimary(theme, phase + 1.30), baseColor, 0.28);
            gradientColors[3] = RenderUtil.ColorUtil.interpolate(ShaderThemeVisuals.animatedSecondary(theme, phase + 1.95), baseColor, 0.28);
        } else {
    ...
    private String resolveShaderTheme(Entity entity) {
        boolean isFriend = entity instanceof AbstractClientPlayerEntity player
                && Strange.get.friendManager.isFriend(player.getGameProfile().getName());
        if (isFriend && friendShader.get()) {
            return friendShaderTheme.get();
        }
        return shaderTheme.get();
    }
