...
public class WaterMark extends Module {
    ...
    public void onScreen(EventScreen e) {
        ...
        if (elements.get(EL_BOSSBAR)) bossBar.render(ctx, bossBarX, bossBarY);
    ...
    private void clampLayoutToScreen(int sw, int sh) {
        ...
        if (oldWatermarkX != watermarkX
                || oldWatermarkY != watermarkY
                || oldModulesX != modulesX
                || oldModulesY != modulesY
                || oldTargetX != targetX
                || oldTargetY != targetY
                || oldInventoryX != inventoryX
                || oldInventoryY != inventoryY
                || oldPotionsX != potionsX
                || oldPotionsY != potionsY
                || oldCooldownsX != cooldownsX
                || oldCooldownsY != cooldownsY
                || oldCoordsX != coordsX
                || oldCoordsY != coordsY
                || oldBindsX != bindsX
                || oldBindsY != bindsY
                || oldArmorX != armorX
                || oldArmorY != armorY
                || oldBossBarX != bossBarX
                || oldBossBarY != bossBarY
                || oldMusicBarX != musicBarX
                || oldMusicBarY != musicBarY) {
            markLayoutDirty();
        }
    }
    ...
    private void renderEditor(DrawContext ctx) {
        ...
        if (elements.get(EL_POTIONS) && potion.getDisplayCount() > 0) {
            ...
            drawBorder(ctx, potionsX, potionsY, pw, ph, ModLocalization.raw("Potions"));
        }
        ...
        if (elements.get(EL_BOSSBAR)) {
            drawBorder(ctx, bossBarX, bossBarY, BossBarHudRenderer.W, BossBarHudRenderer.H, ModLocalization.raw("BossBar"));
        }
    ...
    private void drawBorder(DrawContext ctx, float x, float y, float w, float h, String label) {
        ...
        float labelSize = 4f;
        float labelX = x + 4f;
        float labelY = Math.max(6f, y - 5f);

        FontDraw.drawText(FontDraw.FontType.MEDIUM, ctx, label,
            labelX, labelY, (int) labelSize, editorColor());
    }
    ...
    private void pruneSegmentProgress() {
        segProgress.entrySet().removeIf(entry -> !isWatermarkSegmentKey(entry.getKey())
    ...
    private float scaleMouseCoordinate(double rawCoordinate, int scaledSize, int windowSize) {
        if (!Double.isFinite(rawCoordinate) || scaledSize <= 0 || windowSize <= 0) {
            return Float.NaN;
        }
