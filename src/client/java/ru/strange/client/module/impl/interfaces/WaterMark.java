...
public class WaterMark extends Module {
    ...
    private final BindSettings mediaPrevBind = new BindSettings("Медиа назад", GLFW.GLFW_KEY_F7);
    private final BindSettings mediaPlayBind = new BindSettings("Медиа пауза/плей", GLFW.GLFW_KEY_F8);
    private final BindSettings mediaNextBind = new BindSettings("Медиа вперёд", GLFW.GLFW_KEY_F9);
    private boolean mediaPrevLatch;
    private boolean mediaPlayLatch;
    private boolean mediaNextLatch;
    ...
    public WaterMark() {
        ...
        addSettings(mediaPrevBind);
        addSettings(mediaPlayBind);
        addSettings(mediaNextBind);
    ...
    public JsonObject save() {
        ...
        layout.addProperty("watermarkX", watermarkX / sw);
        layout.addProperty("watermarkY", watermarkY / sh);
        layout.addProperty("modulesX", modulesX / sw);
        layout.addProperty("modulesY", modulesY / sh);
        layout.addProperty("targetX", targetX / sw);
        layout.addProperty("targetY", targetY / sh);
        layout.addProperty("inventoryX", inventoryX / sw);
        layout.addProperty("inventoryY", inventoryY / sh);
        layout.addProperty("potionsX", potionsX / sw);
        layout.addProperty("potionsY", potionsY / sh);
        layout.addProperty("cooldownsX", cooldownsX / sw);
        layout.addProperty("cooldownsY", cooldownsY / sh);
        layout.addProperty("coordsX", coordsX / sw);
        layout.addProperty("coordsY", coordsY / sh);
        layout.addProperty("bindsX", bindsX / sw);
        layout.addProperty("bindsY", bindsY / sh);
        layout.addProperty("armorX", armorX / sw);
        layout.addProperty("armorY", armorY / sh);
        layout.addProperty("bossBarX", bossBarX / sw);
        layout.addProperty("bossBarY", bossBarY / sh);
        layout.addProperty("musicBarX", musicBarX / sw);
        layout.addProperty("musicBarY", musicBarY / sh);
    ...
    public boolean load(JsonObject object) {
        ...
        if (object != null && object.has("HudLayout") && object.get("HudLayout").isJsonObject()) {
            ...
            for (String k : new String[]{"coordsX", "bindsX", "armorX", "bossBarX", "musicBarX", "watermarkX"}) {
                if (layout.has(k) && layout.get(k).getAsFloat() > 1.5f) {
                    oldFormat = true;
                    break;
                }
            }
            ...
            if (oldFormat) {
                ...
                watermarkX  = readFloat(layout, "watermarkX", watermarkX / Math.max(sw, 1)) * sw;
                watermarkY  = readFloat(layout, "watermarkY", watermarkY / Math.max(sh, 1)) * sh;
                modulesX    = readFloat(layout, "modulesX", modulesX / Math.max(sw, 1)) * sw;
                modulesY    = readFloat(layout, "modulesY", modulesY / Math.max(sh, 1)) * sh;
                targetX     = readFloat(layout, "targetX", targetX / Math.max(sw, 1)) * sw;
                targetY     = readFloat(layout, "targetY", targetY / Math.max(sh, 1)) * sh;
                inventoryX  = readFloat(layout, "inventoryX", inventoryX / Math.max(sw, 1)) * sw;
                inventoryY  = readFloat(layout, "inventoryY", inventoryY / Math.max(sh, 1)) * sh;
                potionsX    = readFloat(layout, "potionsX", potionsX / Math.max(sw, 1)) * sw;
                potionsY    = readFloat(layout, "potionsY", potionsY / Math.max(sh, 1)) * sh;
                cooldownsX  = readFloat(layout, "cooldownsX", cooldownsX / Math.max(sw, 1)) * sw;
                cooldownsY  = readFloat(layout, "cooldownsY", cooldownsY / Math.max(sh, 1)) * sh;
                coordsX     = readFloat(layout, "coordsX", coordsX / Math.max(sw, 1)) * sw;
                coordsY     = readFloat(layout, "coordsY", coordsY / Math.max(sh, 1)) * sh;
                bindsX      = readFloat(layout, "bindsX", bindsX / Math.max(sw, 1)) * sw;
                bindsY      = readFloat(layout, "bindsY", bindsY / Math.max(sh, 1)) * sh;
                armorX      = readFloat(layout, "armorX", armorX / Math.max(sw, 1)) * sw;
                armorY      = readFloat(layout, "armorY", armorY / Math.max(sh, 1)) * sh;
                bossBarX    = readFloat(layout, "bossBarX", bossBarX / Math.max(sw, 1)) * sw;
                bossBarY    = readFloat(layout, "bossBarY", bossBarY / Math.max(sh, 1)) * sh;
                musicBarX   = readFloat(layout, "musicBarX", musicBarX / Math.max(sw, 1)) * sw;
                musicBarY   = readFloat(layout, "musicBarY", musicBarY / Math.max(sh, 1)) * sh;
            }
    ...
    public void onUpdate(EventUpdate event) {
        ...
        boolean prevDown = BindUtil.isDown(mediaPrevBind.get());
        if (prevDown && !mediaPrevLatch) { MediaUtil.skipPrevious(); mediaPrevLatch = true; }
        else if (!prevDown) mediaPrevLatch = false;

        boolean playDown = BindUtil.isDown(mediaPlayBind.get());
        if (playDown && !mediaPlayLatch) { MediaUtil.togglePlayPause(); mediaPlayLatch = true; }
        else if (!playDown) mediaPlayLatch = false;

        boolean nextDown = BindUtil.isDown(mediaNextBind.get());
        if (nextDown && !mediaNextLatch) { MediaUtil.skipNext(); mediaNextLatch = true; }
        else if (!nextDown) mediaNextLatch = false;
    }
    ...
    private void initPositions(int sw, int sh) {
        ...
        inventoryX = (sw - InventoryHudRenderer.W) / 2f;
        inventoryY = (sh - InventoryHudRenderer.H) / 2f;
