...
public class GPS extends Module {
    ...
    private final Map<Long, Vec3d> surfacePointCache = new HashMap<>();
    ...
    public GPS() {
        addSettings(colorSetting, markerSize, markerColorMode, markerShaderTheme, lineEnabled, lineMode, lineWidth);
    }
    ...
    private void rebuildRoute(boolean announce) {
        ...
        List<Vec3d> route = blockMode
                ? buildBlockRoute(start, routeGoal)
                : buildSurfaceRoute(start, routeGoal, false);
    ...
    private List<Vec3d> buildSurfaceRoute(Vec3d start, Vec3d goal, boolean blockMode) {
        ...
        double stepDistance = resolveStepDistance(horizontalDistance, blockMode);
        int maxSamples = blockMode ? MAX_BLOCK_SAMPLES : MAX_OPTIMIZED_SAMPLES;
        int steps = Math.max(2, Math.min(maxSamples, (int) Math.ceil(horizontalDistance / stepDistance)));
        double maxFallPerStep = blockMode ? BLOCK_MAX_FALL_PER_STEP : OPTIMIZED_MAX_FALL_PER_STEP;
        ...
        for (int i = 1; i < steps; i++) {
            ...
            Vec3d nextPoint = selectBestRoutePoint(desiredX, desiredZ, previous, goal, blockMode);
            nextPoint = softenDrop(previous, nextPoint, maxFallPerStep);
    ...
    private List<Vec3d> buildBlockRoute(Vec3d start, Vec3d goal) {
        List<Vec3d> route = new ArrayList<>();
        route.add(start);
    ...
    private double resolveStepDistance(double horizontalDistance, boolean blockMode) {
        if (blockMode) {
            return Math.max(1.0D, horizontalDistance / MAX_BLOCK_SAMPLES);
        }
    ...
    private void postProcessRoute(List<Vec3d> route, boolean blockMode) {
        ...
        List<Vec3d> compacted = new ArrayList<>(route.size());
        Vec3d last = null;
        ...
        if (!blockMode) {
            simplifyRoute(route, false);
        }
    }

    private void simplifyRoute(List<Vec3d> route, boolean blockMode) {
        if (route.size() < 3) {
            return;
        }

        List<Vec3d> simplified = new ArrayList<>();
        int lookAhead = blockMode ? 3 : 8;
        int index = 0;
        ...
        while (index < route.size() - 1) {
            ...
            for (int candidateIndex = maxIndex; candidateIndex > index + 1; candidateIndex--) {
                if (canShortcut(route.get(index), route.get(candidateIndex), blockMode)) {
    ...
    private boolean canShortcut(Vec3d start, Vec3d end, boolean blockMode) {
        double horizontalDistance = horizontalDistance(start, end);
        double maxShortcutDistance = blockMode ? 2.75D : 9.0D;
        if (horizontalDistance > maxShortcutDistance) {
            return false;
        }

        int samples = Math.max(1, (int) Math.ceil(horizontalDistance));
        double clearance = blockMode ? 0.05D : 0.20D;
    ...
    private Vec3d computeSurfacePoint(int x, int z, double fallbackY) {
        ...
        for (int y = searchStartY; y >= Math.max(bottomY, searchStartY - 8); y--) {
