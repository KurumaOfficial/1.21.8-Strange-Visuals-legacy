...
//            if (!RenderUtil.isInFieldOfView(mc, p.x, p.y, p.z)) continue;
    ...
    public static class Particle {
        ...
        public Particle(Vec3d pos, float h) {
            ...
            this.phase = (float) random.nextDouble(0.0, 100.0);
            double radius = random.nextDouble(2.0, 5.0);
            double angle = random.nextDouble(0.0, Math.PI * 2.0);
            this.x = pos.x + Math.cos(angle) * radius;
            this.z = pos.z + Math.sin(angle) * radius;
            this.y = pos.y + 2.0 + random.nextDouble(0.0, h + 2.0);
            this.prevX = this.x;
            this.prevY = this.y;
            this.prevZ = this.z;
            this.mX = random.nextDouble(-0.5, 0.5) * 0.001;
            this.mY = random.nextDouble(-0.5, 0.5) * 0.001;
            this.mZ = random.nextDouble(-0.5, 0.5) * 0.001;
        ...
        public void update(long now) {
            ...
            long dt = lastUpdateTime > 0 ? now - lastUpdateTime : 16L;
            if (dt <= 0) dt = 1;
            float dtFactor = dt / 16.0f;
            lastUpdateTime = now;
            ...
            if (velMagSq > 0.0001) {
                if (isHit(x + mX * dtFactor, y, z)) {
                    mX *= -0.8;
                } else {
                    x += mX * dtFactor;
                }
                if (isHit(x, y + mY * dtFactor, z)) {
                    mY *= -0.8;
                } else {
                    y += mY * dtFactor;
                }
                if (isHit(x, y, z + mZ * dtFactor)) {
                    mZ *= -0.8;
                } else {
                    z += mZ * dtFactor;
                }
