...
public class FakePlayer extends Module {
    ...
    public void onAttack(EventAttack event) {
        ...
        float cooldownMs = baseCooldownMs * attackCooldownMultiplier;
        float elapsedMs = (float) (now - lastAttackTime);
        float strength = MathHelper.clamp(elapsedMs / cooldownMs, 0f, 1f);

        float minimumStrength = MathHelper.clamp(
                0.86f + (attackCooldownMultiplier - MIN_ATTACK_COOLDOWN_MULTIPLIER) * 0.35f,
                0.86f,
                0.95f
        );
    ...
    private float calculateLocalDamage(PlayerEntity player, float strength) {
        ...
        float scaledStrength = 0.2f + strength * strength * 0.8f;
        return Math.max(0.1f, base * scaledStrength);
    }
