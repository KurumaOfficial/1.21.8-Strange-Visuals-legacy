...
public class FakePlayer extends Module {
    ...
    private static final float MIN_ATTACK_COOLDOWN_MULTIPLIER = 0.88f;
    private static final float MAX_ATTACK_COOLDOWN_MULTIPLIER = 1.12f;
    ...
    private float            attackCooldownMultiplier = 1.0f;
    ...
    public void onAttack(EventAttack event) {
        ...
        float baseCooldownMs = 1000.0f / Math.max(0.05f, attackSpeed);
        // Per-hit cooldown variation so consecutive attacks land on different strengths.
        float perHitJitter = 0.85f + mc.player.getRandom().nextFloat() * 0.30f;
        float cooldownMs = baseCooldownMs * attackCooldownMultiplier * perHitJitter;
        float elapsedMs = (float) (now - lastAttackTime);
        float strength = MathHelper.clamp(elapsedMs / cooldownMs, 0f, 1f);

        float minimumStrength = 0.40f;
        ...
        applyClientAttackCooldown(attackSpeed);

        boolean fullStrength = strength > 0.9f;
        boolean crit         = fullStrength && isCrit(mc.player);
        boolean sweeping     = fullStrength && mc.player.isOnGround() && mc.player.isSprinting();

        float damage = calculateLocalDamage(mc.player, strength);
        float healthBefore = fake.getHealth();
        applyLocalDamage(damage, crit, sweeping, fullStrength);
        float healthAfter = fake.getHealth();
        boolean killed = healthAfter <= 0f || isDead;
        boolean damaging = healthAfter < healthBefore - 0.001f || killed;

        // Fire confirmed event so HitSound and other listeners work with FakePlayer
        EventManager.call(EventAttack.confirmed(
                fake, healthBefore, healthAfter, strength,
                crit, sweeping, damaging, killed
        ));
    }
    ...
    private void spawn() {
        ...
        GameProfile profile = new GameProfile(fakeUUID, "fake");
        ...
        fake.setCustomName(Text.literal("fake"));
        ...
        fake.deathTime = 0;
        int cooldownBand = Math.abs(fakeUUID.hashCode()) % 25;
        attackCooldownMultiplier = MIN_ATTACK_COOLDOWN_MULTIPLIER + cooldownBand / 100f;
    ...
    private float calculateLocalDamage(PlayerEntity player, float strength) {
        ...
        float scaledStrength = 0.22f + strength * strength * 0.78f;
        // Wider per-hit jitter (±18%) so visible damage numbers vary noticeably per attack.
        float jitter = 0.82f + player.getRandom().nextFloat() * 0.36f;
        // Occasional small armor-pierce overshoot for visual variety.
        if (player.getRandom().nextFloat() < 0.10f) {
            jitter *= 1.10f + player.getRandom().nextFloat() * 0.15f;
        }
        return Math.max(0.1f, base * scaledStrength * jitter);
    }

    private void applyClientAttackCooldown(float attackSpeed) {
        if (!(mc instanceof MinecraftClientAccessor accessor)) {
            return;
        }

        int cooldownTicks = Math.max(1, Math.round(20.0f / Math.max(0.1f, attackSpeed)));
        accessor.setAttackCooldown(cooldownTicks);
    }
