...
public class HitBubble extends Module {
    ...
    public void onRender(EventRender3D event) {
        ...
        try {
            ...
            ShaderThemePreset preset = isShaderRender() ? ShaderThemePreset.byName(shaderTheme.get()) : null;
            for (Bubble bubble : bubbles) {
                if (preset != null) {
                    bubble.renderThemed(matrices, buffer, cameraPos, cameraRotation, preset);
                } else {
    ...
    private final class Bubble {
        ...
        private Bubble(Vec3d pos) {
            this.pos = pos;
            anim.set(0.0);
            anim.run(1.0, LIFE_TIME_MS / 1_000.0, Easings.QUAD_OUT, false);
        }
        ...
        private float getProgress() {
            ...
            return MathHelper.clamp(age / (float) LIFE_TIME_MS, 0.0f, 1.0f);
        }

        private float getRotationDegrees() {
            return getProgress() * 520.0f;
        }
        ...
        private void renderThemed(MatrixStack matrices, VertexConsumer buffer, Vec3d cameraPos, Quaternionf cameraRotation, ShaderThemePreset preset) {
            float alpha = getAlpha();
            if (alpha <= 0.01f) {
                return;
            }

            double phase = pos.x * 0.17 + pos.z * 0.11 + getProgress() * Math.PI * 2.4;
            int base = ShaderThemeVisuals.animatedPrimary(preset.displayName(), phase);
            int accent = ShaderThemeVisuals.animatedSecondary(preset.displayName(), phase + 0.8);
            float mix = 0.5f + 0.5f * (float) Math.sin(phase * 1.3);
            int color = RenderUtil.ColorUtil.interpolate(base, accent, mix);
            renderInternal(matrices, buffer, cameraPos, cameraRotation, RenderUtil.ColorUtil.multAlpha(color, alpha));
        }
        ...
        private void renderInternal(MatrixStack matrices, VertexConsumer buffer, Vec3d cameraPos, Quaternionf cameraRotation, int color) {
            float progress = getProgress();
            float scaleEase = (float) Easings.CUBIC_OUT.ease(Math.min(1.0f, progress / 0.58f));
            float scale = 0.82f * (0.72f + scaleEase * 0.62f);

            matrices.push();
            matrices.translate(pos.x - cameraPos.x, pos.y - cameraPos.y, pos.z - cameraPos.z);
            matrices.multiply(cameraRotation);
            matrices.multiply(net.minecraft.util.math.RotationAxis.POSITIVE_Z.rotationDegrees(getRotationDegrees()));
