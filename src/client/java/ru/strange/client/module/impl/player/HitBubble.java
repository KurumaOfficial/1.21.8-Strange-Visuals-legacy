...
public class HitBubble extends Module {
    ...
    public void onAttack(EventAttack event) {
        if (!event.isConfirmed() || mc.player == null || mc.world == null) {
            return;
        }
    ...
    public void onRender(EventRender3D event) {
        if (mc.player == null || mc.world == null || bubbles.isEmpty()) {
            return;
        }
        ...
        if (isShaderRender()) {
            renderShaderBubbles(texture, matrices, cameraPos, partialTicks);
            return;
        }
        ...
        try {
            ...
            for (Bubble bubble : bubbles) {
                bubble.renderTextured(matrices, buffer, cameraPos, partialTicks);
            }
            immediate.draw();
        } finally {
            allocator.close();
    ...
    private void renderShaderBubbles(Identifier texture, MatrixStack matrices, Vec3d cameraPos, float partialTicks) {
        ShaderThemePreset preset = ShaderThemePreset.byName(shaderTheme.get());
        MaskedCosmosRenderer.getInstance().render(
                immediate -> {
                    VertexConsumer maskBuffer = immediate.getBuffer(createMaskLayer(texture));
                    for (Bubble bubble : bubbles) {
                        bubble.renderMask(matrices, maskBuffer, cameraPos, partialTicks);
                    }
                },
    ...
    private final class Bubble {
        ...
        private void renderTextured(MatrixStack matrices, VertexConsumer buffer, Vec3d cameraPos, float partialTicks) {
            renderInternal(matrices, buffer, cameraPos, partialTicks, RenderUtil.ColorUtil.multAlpha(colorSetting.getRGB(), getAlpha()));
        }

        private void renderMask(MatrixStack matrices, VertexConsumer buffer, Vec3d cameraPos, float partialTicks) {
            renderInternal(matrices, buffer, cameraPos, partialTicks, RenderUtil.ColorUtil.multAlpha(0xFFFFFFFF, getAlpha()));
        }

        private void renderInternal(MatrixStack matrices, VertexConsumer buffer, Vec3d cameraPos, float partialTicks, int color) {
