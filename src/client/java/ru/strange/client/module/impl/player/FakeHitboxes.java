...
public class FakeHitboxes extends Module {
    ...
    public void onRender(EventRender3D event) {
        ...
        try {
            if (mode.is("Под прицелом")) {
                renderTargeted(event.getMatrixStack(), immediate, event.getTickDelta());
            } else {
                for (Entity entity : mc.world.getEntities()) {
                    if (!shouldRenderOverlayEntity(entity)) {
                        continue;
                    }
                    renderExpandedBox(event.getMatrixStack(), immediate, entity, event.getTickDelta());
                }
            }
    ...
    private void renderTargeted(MatrixStack matrices, VertexConsumerProvider.Immediate immediate, float tickDelta) {
        Entity target = getTargetedEntity();
        if (target == null || !shouldRenderOverlayEntity(target)) {
            return;
        }

        renderExpandedBox(matrices, immediate, target, tickDelta);
    }

    private void renderExpandedBox(MatrixStack matrices, VertexConsumerProvider.Immediate immediate, Entity target, float partialTicks) {
        ...
        Box box = getExpandedBoundingBox(target);
        ...
        int[] gradient = new int[] {
                RenderUtil.ColorUtil.multDark(baseColor, 0.42f),
                RenderUtil.ColorUtil.multDark(baseColor, 0.66f),
                RenderUtil.ColorUtil.multDark(baseColor, 0.42f),
                RenderUtil.ColorUtil.multDark(baseColor, 0.66f)
        };
        ...
        if (fill.get()) {
            ...
            Render3D.drawBoxFill(fillBuffer, matrix, minX, minY, minZ, maxX, maxY, maxZ, gradient, 75);
        }

        VertexConsumer lineBuffer = immediate.getBuffer(BOX_LINE_LAYER);
        Render3D.drawBoxOutline(lineBuffer, matrix, minX, minY, minZ, maxX, maxY, maxZ, gradient, 255, 0.18, 0.09);
    }
    ...
    private boolean shouldRenderOverlayEntity(Entity entity) {
