...
public class FakeHitboxes extends Module {
    ...
    private final BooleanSetting shaderFill = new BooleanSetting("Shader Fill", false)
            .hidden(() -> !usesThemeStyle());
    ...
    public FakeHitboxes() {
        addSettings(mode, colorStyle, shaderTheme, shaderFill, expand, range, fill, colorSetting, hitColor);
    }
    ...
    private void renderExpandedBox(MatrixStack matrices, VertexConsumerProvider.Immediate immediate, Entity target,
        ...
        if (shaderFill.get() && usesThemeStyle()) {
            double phase = System.currentTimeMillis() * 0.0016 + x * 0.28 + y * 0.16 + z * 0.22;
            gradientColors[0] = RenderUtil.ColorUtil.interpolate(ShaderThemeVisuals.animatedPrimary(shaderTheme.get(), phase), baseColor, 0.25);
            gradientColors[1] = RenderUtil.ColorUtil.interpolate(ShaderThemeVisuals.animatedSecondary(shaderTheme.get(), phase + 0.8), baseColor, 0.25);
            gradientColors[2] = RenderUtil.ColorUtil.interpolate(ShaderThemeVisuals.animatedPrimary(shaderTheme.get(), phase + 1.6), baseColor, 0.25);
            gradientColors[3] = RenderUtil.ColorUtil.interpolate(ShaderThemeVisuals.animatedSecondary(shaderTheme.get(), phase + 2.4), baseColor, 0.25);
        } else {
    ...
    private boolean shouldOverrideDebugEntity(Entity entity) {
        return entity != null && !entity.isRemoved() && !entity.isInvisible();
    }
    ...
    public static Box getDebugBoundingBox(Entity entity) {
        FakeHitboxes module = getInstance();
        if (module == null || entity == null || entity.isInvisible()) {
            return entity.getBoundingBox();
        }
        return module.getExpandedBoundingBox(entity);
    }
