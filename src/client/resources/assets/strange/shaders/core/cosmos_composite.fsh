...
void main() {
    ...
    float directMaskRect = pad.z;
    float mask;
    vec2 maskUv = sceneUv;
    float maskAlphaScale = clamp(pad.y, 0.0, 1.0);

    if (directMaskRect > 0.5) {
        vec2 screenPos = vec2(
            texCoord.x * screenParams.x,
            (1.0 - texCoord.y) * screenParams.y
        );
        vec2 rectMin = maskRect.xy;
        vec2 rectSize = max(maskRect.zw, vec2(1.0));
        maskUv = (screenPos - rectMin) / rectSize;
        ...
        mask = texture(MaskSampler, safeUv(MaskSampler, maskUv)).a * maskAlphaScale;
    } else {
        mask = texture(MaskSampler, safeUv(MaskSampler, sceneUv)).r * maskAlphaScale;
    }
    ...
    if (edgeGlow > 0.0) {
        vec2 texel = directMaskRect > 0.5
            ? 1.0 / max(maskRect.zw, vec2(1.0))
            : 1.0 / vec2(screenParams.x, screenParams.y);
        float ml;
        float mr;
        float mt;
        float mb;
        ...
        if (directMaskRect > 0.5) {
            ml = texture(MaskSampler, safeUv(MaskSampler, maskUv + vec2(-texel.x, 0.0))).a * maskAlphaScale;
            mr = texture(MaskSampler, safeUv(MaskSampler, maskUv + vec2(texel.x, 0.0))).a * maskAlphaScale;
            mt = texture(MaskSampler, safeUv(MaskSampler, maskUv + vec2(0.0, texel.y))).a * maskAlphaScale;
            mb = texture(MaskSampler, safeUv(MaskSampler, maskUv + vec2(0.0, -texel.y))).a * maskAlphaScale;
        } else {
            ml = texture(MaskSampler, safeUv(MaskSampler, maskUv + vec2(-texel.x, 0.0))).r * maskAlphaScale;
            mr = texture(MaskSampler, safeUv(MaskSampler, maskUv + vec2(texel.x, 0.0))).r * maskAlphaScale;
            mt = texture(MaskSampler, safeUv(MaskSampler, maskUv + vec2(0.0, texel.y))).r * maskAlphaScale;
            mb = texture(MaskSampler, safeUv(MaskSampler, maskUv + vec2(0.0, -texel.y))).r * maskAlphaScale;
        }
    ...
    if (theme >= 23.5) {
        vec3 sceneAfter = texture(SceneAfterSampler, safeUv(SceneAfterSampler, texCoord)).rgb;
